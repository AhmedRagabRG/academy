...
| academic-terms | name | order, name, startDate, updatedAt | `settings.academicTerms.view` |
...
Create fields: `academicYearId`, `name`, `startDate`, `endDate`, `order`, `status`, and
`expectedAcademicYearVersion`. For N retained siblings, order must be in `1..N+1`; insertion shifts
later siblings. Update accepts parent/name/dates/order plus `expectedVersion` and the expected versions
of affected source/destination years. Changing parent requires a destination order. Response includes
server-resolved `academicYearName` and the resulting positive integer `order`.
...
Every retained term participates in ordering, including archived terms. For N terms in one year, the
orders are exactly `1..N`. Archive/reactivation preserves order. A cross-year move compacts the source
and inserts into the destination atomically. Status-filtered lists can show apparent gaps; the complete
year catalogue is the authoritative contiguous sequence. Default term sorting is `order ASC`, then
`startDate ASC`, then ID.

Changing `order` through the ordinary term PATCH performs one atomic interval move and shifts affected
siblings. Create/move requests version-check the affected year collection(s); the term PATCH also
version-checks the moved term. No additional endpoint or permission is introduced.
