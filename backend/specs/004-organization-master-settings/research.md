...
permissions, academic-term overlap behavior, and the approved explicit academic-term `order` field.
...
## 7A. Academic-Term Explicit Ordering

**Decision**: Every retained term carries a positive integer `order` unique within its academic year,
regardless of active, inactive, or archived status. For N retained terms, the exact order set is
`1..N`. Create inserts at `1..N+1` and shifts later siblings. Updating a term's order performs one
atomic move and shifts the affected interval. Moving between years atomically compacts the source and
inserts into the destination. Archive and reactivation preserve order because retained terms remain members
of the ordered historical catalogue.

**Rationale**: This is the only interpretation that simultaneously keeps an Order on every academic
term, retains archived history, and guarantees unique contiguous ordering after every lifecycle
operation. It avoids nullable/sentinel archived positions and preserves historical sequence.

**Alternatives considered**: Sequence only non-archived terms and set archived order to null (changes
the approved entity contract); preserve archived order while compacting live terms (creates duplicate
positions); derive from start date (rejected by owner); allow gaps or fractional ranks (violates the
approved contiguous integer rule).

## 7B. Term-Order Concurrency and Contract

**Decision**: Serialize membership/order changes by locking the owning AcademicYear row; cross-year
moves lock both parents in stable ID order. Use a deferrable unique `(academicYearId,order)` database
constraint and `order > 0` check; validate contiguity under the parent lock before commit. The existing
year version serves as the aggregate order revision. Create and single-term move requests carry the
current affected year version(s), while the moved term also carries its own expected version.

**Rationale**: Individual term versions cannot detect stale collection membership. Parent revisions,
stable lock order, and deferred uniqueness make inserts and interval shifts atomic and concurrency-safe
without adding an endpoint that the owner did not request.

**Alternatives considered**: A complete bulk-reorder endpoint (not required by the approved scope and
absent from the canonical contract); term versions only (cannot detect a newly inserted sibling);
immediate uniqueness with temporary negative ranks (conflicts with the positive-order invariant); a
separate order-version column (unnecessary when the year version can own child-membership concurrency).
