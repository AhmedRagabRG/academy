...
export function UserProfileScreen({ userId }: { userId: string }) {
  ...
  const permissions = useEffectivePermissions(userId)

  return (
    <SettingsPage
      title={user.data?.fullName ?? "ملف المستخدم"}
      description="تفاصيل الحساب والصلاحيات الفعالة الموروثة من الأدوار."
    >
      <AdministrativeQueryState
        loading={user.isLoading || permissions.isLoading}
        error={user.error ?? permissions.error}
      >
        {user.data && (
            ...
            <Card>
              ...
              <dl className="mt-5 grid gap-4 text-sm">
                <div>
                  ...
                  <dd>
                    <bdi>{user.data.email}</bdi>
                  </dd>
                </div>
                <div>
                  <dt className="text-muted-foreground">الهاتف</dt>
                  <dd>
                    <bdi>{user.data.phone}</bdi>
                  </dd>
                </div>
              </dl>
            </Card>
