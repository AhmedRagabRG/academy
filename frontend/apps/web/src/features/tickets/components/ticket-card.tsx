...
export function TicketCard({ ticket, onMove }: { ticket: TicketSummary; onMove: (status: Exclude<TicketSummary["status"], "archived">) => void }) {
  return <Card className="space-y-3 p-4 shadow-sm transition hover:shadow-md" data-ticket-id={ticket.id}>
    <div className="flex items-start justify-between gap-2"><bdi dir="ltr" className="text-muted-foreground text-xs font-semibold">{ticket.number}</bdi><TicketPriorityBadge priority={ticket.priority} /></div>
    <Link href={`/tickets/${ticket.id}`} className="block font-semibold leading-6 hover:text-primary">{ticket.title}</Link>
    <div className="flex flex-wrap gap-1.5"><Badge>{ticket.departmentName}</Badge>{ticket.tags.map((tag) => <Badge key={tag} className="bg-primary/5">{tag}</Badge>)}</div>
